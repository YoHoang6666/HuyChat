...
// Get the selectedToken value
$selectedToken = $_SESSION['selectedToken'] ?? "None";
?>
<p>Selected Token: <b id="selectedToken"><?= htmlspecialchars($selectedToken); ?></b></p>
...
function setToken(token) {
    ...
    .then(data => {
        console.log("Selected Token:", data.selectedToken); // Debugging
        let tokenElement = document.getElementById("selectedToken");
        if (tokenElement) {
            tokenElement.innerText = data.selectedToken; // Update the UI
        } else {
            console.error("Element with ID 'selectedToken' not found.");
        }
    })
